...
#define PHOTO_NUM_MAX		100 // 照片最大数量
...
int cgiMain()
{
	DIR *dir;
	struct dirent *dirp;
	char photoname[PHOTO_NUM_MAX][48]; 
	struct _syncmsg1 syncmsg1 = {0, 0, 0};     //cgi 中操作的数据
	struct _syncmsg2 syncmsg2 = {{0}, {0}, {0}};//web页面穿的数据
	char path[256] = {0};
	...
	while((dirp = readdir(dir)) != NULL)  //读文件
	{
		if(dirp->d_name[0] > '9' || dirp->d_name[0] < '0')   //名称首字母
			continue;
		sprintf(photoname[syncmsg1.photo_num++], "%s", dirp->d_name);
		if(syncmsg1.photo_num >= PHOTO_NUM_MAX)         
		{
	...
	cgiFormString("number", syncmsg2.number,13);      //网页传来的数据
	cgiFormString("index", syncmsg2.index,13);				//网页传来的指针
	cgiFormString("action", syncmsg2.action,13);			//网页传的交互标志
	if(syncmsg2.number == NULL || syncmsg2.index == NULL || syncmsg2.action == NULL) //第一次浏览
	{	
		syncmsg1.photo_idx = 0; // 浏览第一张指针
		syncmsg1.key_action = 0; // 浏览第一张行为
	}
	else
	{
